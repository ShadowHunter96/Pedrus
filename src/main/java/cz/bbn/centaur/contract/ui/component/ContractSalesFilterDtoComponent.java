...
public class ContractSalesFilterDtoComponent extends AppAdvancedFilter {

    private ComboBox<SubjectDto> subject;
    ...
    private MultiSelectComboBox<ContractInternalType> type;
    ...
    public ContractSalesFilterDtoComponent(Button search, List<SubjectDto> subjectDtoList,
                                           List<EnumerationDto> contractStateDtoList,
                                           List<ContractTypeDto> contractTypeDtoList,
                                           List<UserDto> userList, List<AreaDto> areaDtoList,
        ...
        super(search);
        this.customerDtoList = subjectDtoList;
    ...
    private void initComponents() {
        subject = new ComboBox<>(Transl.get("Subject"));
        List<SubjectDto> subjectList = new ArrayList<>(customerDtoList);
        subjectList.sort(Comparator.comparing(SubjectDto::getName));
        subject.setItems(subjectList);
        subject.setItemLabelGenerator(SubjectDto::getName);
        addToBasicFilter(subject);
        ...
        type = new MultiSelectComboBox<>(Transl.get("Internal type"));
        EnumSet<ContractInternalType> typeSet = EnumSet.of(ContractInternalType.SALES, ContractInternalType.SUPPLIER);
        type.setItems(typeSet);
        type.setValue(typeSet);
        type.setItemLabelGenerator(item -> Transl.get(item.name() + " contract"));
        addToBasicFilter(type);
    ...
    private void fillFilterFromUrl() {
        ...
        if (map.containsKey("type")) {
            String[] typeStrArr = map.get("type").split(",");
            Set<ContractInternalType> typeSet = new HashSet<>();
            for (String typeStr : typeStrArr) {
                if (ContractInternalType.SALES.name().equals(typeStr)
                        || ContractInternalType.SUPPLIER.name().equals(typeStr)) {
                    typeSet.add(ContractInternalType.valueOf(typeStr));
                }
            }
            type.setValue(typeSet);
        }
    ...
    public void fillUrl() {
        ...
        if (subject.getValue() != null) {
            paramUrl = paramUrl.concat("&customer=").concat(subject.getValue().getId());
        }
        ...
        if (type.getValue() != null && !type.getValue().isEmpty()) {
            paramUrl = paramUrl.concat("&type=");
            Set<String> typeStrSet = new HashSet<>();
            for (ContractInternalType intType : type.getValue()) {
                typeStrSet.add(intType.name());
            }
            paramUrl = paramUrl.concat(String.join(",", typeStrSet));
        }
    ...
    public ContractFilterDto getContractFilterDto() {
        ...
        dto.setCustomerDto(subject.getValue());
